...
parser.add_argument('--nIter', type=int, default=4000)
parser.add_argument('--imgSize', type=int, default=64)
parser.add_argument('--lam', type=float, default=0.1)

parser.add_argument('--checkpointDir', type=str, default='./checkpoint_200')
...
parser.add_argument('--maskType', type=str,
                    choices=['random', 'center', 'left', 'full', 'grid', 'lowres', 'file'],
                    default='file')
parser.add_argument('--inmaskfile', type=str, 
                    default=None)
...
# assert(os.path.exists(args.inmaskfile))

config = tf.ConfigProto() # manipulate GPU
...
with tf.Session(config=config) as sess:
    dcgan = DCGAN(sess, image_size=args.imgSize,
                  ...
                  inmaskfile_dir=args.inmaskfile)
    dcgan.complete(args)
